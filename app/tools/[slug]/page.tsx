    ...
    >
      <div className={styles.detailsGrid}>
        ...
        <section className={styles.sectionCard}>
          ...
          <ul className={styles.relatedList}>
            {seoContent.relatedTools.map(
              (relatedTool) => (
                <li key={relatedTool.slug}>
                  <Link
                    href={
                      `/tools/${relatedTool.slug}` as Route
                    }
                  >
                    {relatedTool.name} tool
                  </Link>
                </li>
              )
            )}
...
export default async function Page({
  ...
  if (tool) {
    ...
    const toolCategoryId =
      getToolCategoryBySlug(slug);

    const categoryTitle = toolCategoryId
      ? categoryTitles[toolCategoryId]
      : null;

    const categoryPath = toolCategoryId
      ? `/tools/${toolCategoryId}`
      : null;
    ...
    const breadcrumbItems = [
      ...
      ...(toolCategoryId
        ? [
            {
              name: categoryTitles[toolCategoryId],
              item: `https://easemytools.com/tools/${toolCategoryId}`,
            },
          ]
        : []),
      {
        name: tool.name,
        item: `https://easemytools.com/tools/${slug}`,
      },
    ];
    ...
    const faqSchema = tool.seoContent?.faqs
      ? createFAQSchema(
