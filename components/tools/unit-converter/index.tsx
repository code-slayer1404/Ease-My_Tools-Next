...
const UnitConverter = () => {
    ...
    const getUnits = () => {
        ...
        return unitKeys.map((key) => ({
            value: key,
            label: unitLabels[category]?.[key] || key.charAt(0).toUpperCase() + key.slice(1)
        }));
    };
        ...
        setTimeout(() => {
            try {
                if (category === 'temperature') {
                    ...
                    const converter = temperatureFormulas[fromUnit as keyof typeof temperatureFormulas];
                    ...
                    const categoryFormulas = conversionFormulas[category] as Record<string, number>;
    ...
    return (
        <div className={styles["unit-converter"]}>
            ...
            <div className={styles["converter-container"]}>
                <div className={styles["category-selector"]}>
                    {(Object.keys(categoryLabels) as ConversionCategory[]).map((key) => (
                        <button
                            key={key}
                            className={`${styles["category-btn"]} ${category === key ? styles["active"] : ""}`}
                ...
                {result && result !== 'Error' && (
                    <div className={styles["result-display"]}>
                        <h3>Result:</h3>
                        <p>
                            {inputValue} {unitLabels[category]?.[fromUnit] || fromUnit} = 
                            <strong> {result} {unitLabels[category]?.[toUnit] || toUnit}</strong>
                        </p>
                    </div>
                )}
