...
const JsonTool: React.FC<ToolHeroProps> = ({ tool }) => {
    const [inputJson, setInputJson] = useState<string>(
        `{\n  "name": "JSON Tool",\n  "version": "1.0",\n  "features": ["validate", "format", "minify"]\n}`
    )
    const [output, setOutput] = useState<string>("")
    ...
    return (
        <div className="flex flex-col justify-center bg-gradient-to-br from-slate-50 via-white to-slate-100 px-3 py-8 text-slate-900 sm:px-4 sm:py-10 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 dark:text-slate-100">
            <div className="w-full max-w-7xl space-y-8 mx-auto">
                {/* Platform UI synchronized header integration */}
                <ToolHero tool={tool} />
