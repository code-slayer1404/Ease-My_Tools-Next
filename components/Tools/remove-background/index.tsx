...
// // Simple debounce helper (unchanged)
...
// export default function BackgroundRemover({ tool }: ToolHeroProps) {
//   const {
//     queue,
//     removeBackground,
//     isProcessing,
...
//             {/* Header area (engine status, actions) */}
//             <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
//               <div className="flex items-center gap-2">
//                 {engine.device && (
//                   <Badge
//                     variant={
//                       engine.device === "webgpu" ? "default" : "secondary"
//                     }
//                     className="whitespace-nowrap"
//                   >
//                     {engine.device === "webgpu" ? "⚡ WebGPU" : "🧠 WASM"}
...
//                     onChange={handleFileChange}
//                   />
//                   <div className="mb-4 rounded-2xl border border-border bg-muted p-5 shadow-inner">
//                     <Upload className="h-8 w-8 text-indigo-600 dark:text-indigo-400" />
//                   </div>
//                   <h3 className="text-xl font-bold text-foreground">
...
//                   </h3>
//                   <p className="mt-2 max-w-sm text-sm text-muted-foreground">
//                     Process a single photo or a massive batch directly in your browser.
//                   </p>
//                 </CardContent>
//               </Card>
//             )}
...
//                           URL.revokeObjectURL(activeItem.resultUrl)
//                         updateItemResultUrl(activeItem.id, newUrl)
//                       }}
//                     />
//                   </div>
...
// /* ============ InteractiveCanvasEditor – FULLY INCLUDED ============ */
// interface EditorProps {
//   item: ProcessedItem
//   onSaveMask: (newUrl: string) => void
// }
...
/* ============ InteractiveCanvasEditor – fully repaired history state ============ */
interface EditorProps {
  item: ProcessedItem
  onSaveMask: (newUrl: string) => void
}
...
function InteractiveCanvasEditor({ item, onSaveMask }: EditorProps) {
  ...
  // Swapped to raw ImageData states instead of blob urls to bypass asynchronous loading loop bugs
  const [history, setHistory] = useState<ImageData[]>([])
  const [historyIndex, setHistoryIndex] = useState<number>(-1)
  ...
  const handlePointerUp = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>) => {
      ...
      setHistory((prev) => {
        let updated = [...prev.slice(0, historyIndex + 1), nextFrame]
        if (updated.length > MAX_HISTORY) {
          updated = updated.slice(1)
          setHistoryIndex(updated.length - 1)
        } else {
  ...
  const applyHistoryState = useCallback(
    (index: number) => {
      ...
      // Commit changes downstream asynchronously to prevent cyclical rendering loops
      saveMaskDebounced.current(canvas)
    },
    [history]
  )
